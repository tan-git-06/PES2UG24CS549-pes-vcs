...
int commit_create(const char *message, ObjectID *commit_id_out) {
    Commit commit;
    // Clear the struct to avoid garbage values in author/message arrays
    memset(&commit, 0, sizeof(Commit));

    // 1. Generate the root tree from the index
    // Uses the 'tree' member of the struct
    if (tree_from_index(&commit.tree) != 0) {
        return -1;
    }

    // 2. Handle the parent commit
    ...
    // 3. Set metadata
    // Use the helper to get your name/SRN from the environment
    const char *author_env = pes_author();
    strncpy(commit.author, author_env, sizeof(commit.author) - 1);
    
    commit.timestamp = (uint64_t)time(NULL);
    
    strncpy(commit.message, message, sizeof(commit.message) - 1);

    // 4. Convert struct to text format for storage
    void *raw_data;
    size_t raw_len;
    ...
    // 5. Write the commit object to .pes/objects/
    ...
    // 6. Update the branch pointer (HEAD) to this new commit hash
