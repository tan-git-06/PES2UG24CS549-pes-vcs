...
int index_load(Index *index) {
    ...
    if (!f) return 0; // If file doesn't exist, index is just empty
    ...
    char path_buf[512];
// Correct - limit path to 511 characters
while (fscanf(f, "%o %64s %ld %u %511s", 
    ...
    // Use sizeof to be extra safe during the copy
    strncpy(index->entries[index->count].path, path_buf, sizeof(index->entries[index->count].path) - 1);
...
int index_save(const Index *index) {
    // 1. Sort entries by path (important for Git-like consistency)
...
int index_add(Index *index, const char *path) {
    ...
    // 3. Update or create the index entry
